...
//|                         Crash 900 Index - Trained on 19 months    |
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap Project"
#property link      ""
#property version   "1.00"
#property description "Crash 900 v1 - Trained on 19 months of data"
#property description "Unique: post-crash strong (+12%), 6-7 consec up reversal, squeeze+uptrend"
...
int OnInit()
{
   ...
   Print("Crash900 v1 initialized | Symbol: ", _Symbol);
...
//+------------------------------------------------------------------+
//| Count consecutive UP candles (unique to Crash 900)                |
//+------------------------------------------------------------------+
int CountConsecUp()
{
   int count = 0;
...
bool DetectEntrySignal(string &signalName, double &signalScore)
{
   ...
   // === CRASH 900 SIGNALS - Trained on 19 months + backtest results ===
   //
   // FROM BEHAVIOR DISCOVERY (19 months):
   //   doji_candle             -23.9% edge | 3/3 years | FILTER
   //   crash_recovery          +12.0% edge | 62% rate  | STRONG SIGNAL
   //   post_crash_1_bar        +11.5% edge | 67% rate  | STRONG SIGNAL
   //   6-7 consec up = reversal -8 to -12% | UNIQUE to C900 | AVOID BUY
   //   vol_squeeze             -8.0% edge  | 3/3 years | FILTER if downtrend
   //   RSI 20-30 oversold      +3.8% edge  | 54% rate  | SIGNAL
   //   RSI 70-80 momentum      +3.4% edge  | 53% rate  | SIGNAL
   //   RSI 50-60 neutral       +5.1% edge  | 55% rate  | confirmed 3/3 years
   //
   // FROM BACKTEST (losing combos to block):
   //   squeeze_up+rsi_momentum+h4  25% WR  -$336 | BLOCK
   //   squeeze_up+cross+h4         26% WR  -$201 | BLOCK
   //   rsi_os+consec_rev+overext+h4 23% WR -$169 | BLOCK
   //   rsi_os+overext+post_crash    0% WR  -$90  | BLOCK
   ...
   // Doji filter (3/3 years validated, -24% edge)
   if(IsDoji(1))
   {
   ...
   // UNIQUE TO C900: After 6+ consecutive UP candles = reversal coming
   // DON'T BUY after extended uptrend (-12% to -21% edge)
   if(consecUp >= 6)
   {
      signalName = "FILTER:consec_up_reversal";
      return false;
   }

   // Squeeze in downtrend = no trade (vol_squeeze -8% edge)
   ...
   // === BLOCKS from backtest losing combos ===

   // BLOCK: squeeze_up+rsi_momentum+h4 (25% WR, -$336 biggest loser!)
   ...
   // BLOCK: squeeze_up+cross+h4 (26% WR, -$201)
   if(hasSqueezeUptrend && hasBullCross && hasH4Bull && !hasRSIOS && !hasConsecRev)
   {
      signalName = "BLOCK:squeeze_cross_h4";
      return false;
   }

   // BLOCK: rsi_os+consec_rev+overext+h4 (23% WR, -$169)
   if(hasRSIOS && hasConsecRev && hasOverext && hasH4Bull && !hasPostCrash)
   {
      signalName = "BLOCK:rsi_consec_overext_h4";
      return false;
   }

   // BLOCK: rsi_os+overext+post_crash without H4 (0% WR, -$90)
   if(hasRSIOS && hasOverext && hasPostCrash && !hasH4Bull)
   {
      signalName = "BLOCK:rsi_overext_crash_no_h4";
      return false;
   }

   // === BUILD SIGNALS (ordered by C900 training) ===

   // SIGNAL 1: Post-crash recovery (C900 MAIN EDGE: +12% edge, 62-67%)
   // Unlike C1000 where it weakened, on C900 it's the strongest signal
   if(hasPostCrash)
   {
      signalName += "post_crash+";
      signalScore += 1.2;
      signalCount++;
      hasStrongSignal = true;  // Can enter alone
   }

   // SIGNAL 2: Squeeze + Uptrend (profitable in backtest: +$2091 with H4)
   if(hasSqueezeUptrend && !hasRSIMomentum)  // exclude combo with momentum (loses)
   {
      signalName += "squeeze_up+";
      signalScore += 0.9;
      signalCount++;
      if(!hasStrongSignal) hasStrongSignal = true;
   }

   // SIGNAL 3: RSI Momentum 70-80 + H4 (profitable: +$1555 with H4)
   // BUT NOT with squeeze (blocked above)
   if(hasRSIMomentum && hasUptrend && hasH4Bull && !hasSqueeze)
   {
   ...
   // SIGNAL 4: RSI oversold 20-35 bounce (+3.8% edge, 54%)
   if(hasRSIOS)
   {
      signalName += "rsi_os+";
      signalScore += 0.5;
   ...
   // SIGNAL 5: Consecutive down reversal
   if(hasConsecRev)
   {
   ...
   // SIGNAL 6: Overextended below EMA20
   if(hasOverext)
   {
      signalName += "overext+";
      signalScore += 0.3;
   ...
   // === ENTRY RULES ===
   // Post-crash can enter alone (67% rate, C900's signature)
   // Squeeze+uptrend can enter alone (profitable in backtest)
   // RSI momentum needs H4 (profitable only with H4)
...
void PrintSummaryReport()
{
   ...
   Print("======================================================================");
   Print("  CRASH900 v1 - BACKTEST SUMMARY REPORT");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
