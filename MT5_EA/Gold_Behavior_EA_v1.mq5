...
//
//  TOP 4 LONG recipes (shorts disabled — too weak even at R:R 2:1):
...
#property description "XAUUSD H1 behavioral EA - 4 LONG recipes validated OOS, R:R 2:1."
#property description "v1.30: regime filter (EMA200 slope + optional ADX) to skip choppy markets."
#property strict
...
int OnInit()
{
   ...
   PrintFormat("=== Gold Behavior EA v1.10 init ===");
   ...
   PrintFormat("  Recipes enabled: GL1=%s GL2=%s GL3=%s GL4=%s  (GL3 confluence-req=%s)",
...
MatchResult MatchRecipes(const Conditions &c)
{
   ...
   if(InpGL3RequireConfluence && GL3 && !(GL1 || GL2 || GL4)) GL3 = false;
...
void LogContext(const Conditions &c, string prefix)
{
   ...
   PrintFormat("%s pr[atr=%.2f bbp=%.2f bbw=%.2f de50=%.2f de200=%.2f rsi=%.2f] "
               "flags[atrQ3=%d bbwQ3=%d bbpQ2=%d de200Q4=%d de50Q2=%d rsiQ3=%d h5=%d lon=%d]",
...
void PrintSummaryReport()
{
   ...
   // --- Per-recipe tracking (GL1-GL4) ---
   string recipeNames[4] = {"GL1", "GL2", "GL3", "GL4"};
   int    recipeCount[4] = {0, 0, 0, 0};        // trades where recipe was present
   int    recipeWins[4]  = {0, 0, 0, 0};
   double recipePnl[4]   = {0, 0, 0, 0};
   // Solo-recipe tracking: trades where ONLY that recipe fired (no overlap)
   int    recipeSoloCount[4] = {0, 0, 0, 0};
   int    recipeSoloWins[4]  = {0, 0, 0, 0};
   double recipeSoloPnl[4]   = {0, 0, 0, 0};
   ...
   Print("======================================================================");
   Print("  GOLD BEHAVIOR EA v1.10 - BACKTEST SUMMARY REPORT");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
