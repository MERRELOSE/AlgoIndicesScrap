//+------------------------------------------------------------------+
//|                          WeekendGap_EA_v4_PostOpenCheck.mq5       |
//|     ONNX-gated weekend gap with ACTIVE post-open profit check     |
//|                       AlgoIndicesScrap Project                    |
//+------------------------------------------------------------------+
//  v4 = "capture the gap, BE-protect while we wait" (Kennedy's design):
//   - Friday 19:15-20:00 UTC: open trade (ML-gated, body>=1%)
//   - SL = 0.10% during weekend (catastrophic protection - same as v3)
//
//   At Sunday reopen there are 3 cases:
//      (a) gap < -0.10%  -> SL fires at first tick, recorded loss
//      (b) gap in [-0.10%, 0]  -> small loss zone, no SL
//      (c) gap > 0  -> profit
//
//   Phase 1 (Sunday 22:00 first tick post-open):
//      For each open position:
//        if current price > entry  -> move SL to entry (BREAKEVEN)
//        else                       -> CLOSE NOW (take small loss before
//                                       the M1 wick can worsen it)
//
//   Phase 2 (Sunday 22:01 = 60 sec later, after first M1 bar closes):
//      Close all remaining positions. They're either in profit or
//      already BE-protected, so worst case we exit flat.
//
//   Hypothesis: the gap is the only edge. Post-gap price action is
//   random noise. Get out fast, BE-protect during the unavoidable wait.
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap"
#property version   "4.00"
#property description "Weekend gap EA v4 - active post-open check: cut any loss at Sunday open."
#property strict
...
input int    InpMagic           = 790040;   // v4 magic
...
//| HARDCODED                                                         |
//+------------------------------------------------------------------+
#define FIXED_SL_PCT          0.10     // v4: same as v3 (catastrophic safety, gap-overshoot)
#define FIXED_MIN_BODY_PCT    1.00     // pre-filter even before ML
#define FIXED_LOOKBACK_H      20
#define FIXED_LONG_ONLY       true

// v4: post-open window covers BOTH phases:
//   Phase 1 (BE/cut-loss) on first tick at >=22:00
//   Phase 2 (close-all) >=60 sec after Phase 1
#define FIXED_POSTOPEN_HOUR_UTC  22    // summer; winter +1h auto via DST
#define FIXED_POSTOPEN_MIN_UTC   0     // start 22:00 (the actual reopen)
#define FIXED_POSTOPEN_WIN_MIN   5     // window 22:00-22:05 UTC (5 min, plenty)
#define FIXED_PHASE2_DELAY_SEC   60    // wait this long between Phase 1 and Phase 2
...
// v4 phase tracking - by weekend day-stamp so each weekend gets fresh state
datetime g_phase1_done_day       = 0;   // BE / cut-loss action done for this Sunday
datetime g_phase1_done_time      = 0;   // exact time Phase 1 ran (for Phase 2 delay)
datetime g_phase2_done_day       = 0;   // close-all action done for this Sunday
...
int g_phase1_be_done        = 0;     // v4: positions moved to BE in Phase 1
int g_phase1_cut_loss       = 0;     // v4: positions closed in Phase 1 (small loss)
int g_phase2_closed_profit  = 0;     // v4: positions closed in Phase 2 in profit
int g_phase2_closed_be      = 0;     // v4: positions closed in Phase 2 at BE (SL hit)
...
int OnInit()
{
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v4 POSTOPEN - INIT");
   Print("======================================================================");
...
// v4: post-open check fires in the first minutes after Sunday market reopen
bool IsInPostOpenWindow(const MqlDateTime &dt_utc)
{
   if(dt_utc.day_of_week != FIXED_EXIT_DOW) return false;
   int eff_hour = FIXED_POSTOPEN_HOUR_UTC + _SeasonalShift(dt_utc);
   int now_min = dt_utc.hour * 60 + dt_utc.min;
   int tgt_min = eff_hour * 60 + FIXED_POSTOPEN_MIN_UTC;
   return (now_min >= tgt_min && now_min <= tgt_min + FIXED_POSTOPEN_WIN_MIN);
}
...
//+------------------------------------------------------------------+
//| v4 PHASE 1 - first tick post-Sunday-open                          |
//| For each open position:                                           |
//|   - if currently in profit -> move SL to entry (BE)               |
//|   - if currently in loss   -> close immediately                   |
//+------------------------------------------------------------------+
void Phase1_BE_or_CutLoss(datetime now_utc)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(!g_pos.SelectByIndex(i)) continue;
      ...
      double cur     = is_long ? bid : ask;
      double pnl_pct = is_long ? (cur - entry) / entry * 100.0
                               : (entry - cur) / entry * 100.0;

      if(pnl_pct > 0)
      {
         // BE move: set SL to entry price
         double tp = g_pos.TakeProfit();
         if(g_trade.PositionModify(g_pos.Ticket(), entry, tp))
         {
            g_phase1_be_done++;
            if(InpLogEachWeekend)
               Print("[PHASE1-BE] ", TimeToString(now_utc, TIME_DATE|TIME_MINUTES),
                     " entry=", DoubleToString(entry, g_digits),
                     " cur=", DoubleToString(cur, g_digits),
                     " profit=", DoubleToString(pnl_pct, 4), "% -> SL moved to BE");
         }
         else
            Print("[ERROR] BE modify failed: ", g_trade.ResultRetcode(), " - ",
                  g_trade.ResultRetcodeDescription());
      }
      else
      {
         // Small loss zone - cut immediately, don't wait for M1 wick
         if(g_trade.PositionClose(g_pos.Ticket()))
         {
            g_phase1_cut_loss++;
            if(InpLogEachWeekend)
               Print("[PHASE1-CUT] ", TimeToString(now_utc, TIME_DATE|TIME_MINUTES),
                     " entry=", DoubleToString(entry, g_digits),
                     " cur=", DoubleToString(cur, g_digits),
                     " loss=", DoubleToString(pnl_pct, 4), "% -> closed early");
         }
      }
   }
}
...
//+------------------------------------------------------------------+
//| v4 PHASE 2 - 60 sec after Phase 1 (first M1 bar closed)           |
//| Close all remaining positions (they're profit or BE'd).           |
//+------------------------------------------------------------------+
void Phase2_CloseAll(datetime now_utc)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(!g_pos.SelectByIndex(i)) continue;
      ...
      if(g_trade.PositionClose(g_pos.Ticket()))
      {
         if(pnl_pct > 0) g_phase2_closed_profit++;
         else            g_phase2_closed_be++;
         if(InpLogEachWeekend)
            Print("[PHASE2] ", TimeToString(now_utc, TIME_DATE|TIME_MINUTES),
                  " entry=", DoubleToString(entry, g_digits),
                  " cur=", DoubleToString(cur, g_digits),
                  " final_pnl=", DoubleToString(pnl_pct, 4), "%");
      }
   }
}
...
void OnTick()
{
   ...
   bool in_postopen = IsInPostOpenWindow(dt_utc);
   bool in_exit     = IsInExitWindow(dt_utc);
   if(in_entry) g_ticks_in_entry_window++;
   if(in_exit)  g_ticks_in_exit_window++;

   // v4: 2-phase post-open routine (BE/cut, then close-all 60s later)
   datetime today_stamp = (datetime)(now / 86400);
   if(HasOpenPosition() && in_postopen)
   {
      // Phase 1: first tick of the post-open window
      if(g_phase1_done_day != today_stamp)
      {
         g_phase1_done_day  = today_stamp;
         g_phase1_done_time = now;
         Phase1_BE_or_CutLoss(now);
      }
      // Phase 2: 60 sec after Phase 1 (first M1 bar has closed)
      else if(g_phase2_done_day != today_stamp
              && now - g_phase1_done_time >= FIXED_PHASE2_DELAY_SEC)
      {
         g_phase2_done_day = today_stamp;
         Phase2_CloseAll(now);
      }
   }
...
void TryOpenWeekendTrade(datetime now_utc)
{
   ...
   // SL distance = max of (FIXED_SL_PCT, broker stops_level+50pts, 0.30% absolute floor)
   double sl_dist_pct     = entry_price * FIXED_SL_PCT / 100.0;
   long   min_stop_pts    = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   double sl_dist_broker  = (min_stop_pts + 50) * g_point;
   double sl_dist_min_pct = entry_price * 0.30 / 100.0;
   double sl_dist = MathMax(sl_dist_pct, MathMax(sl_dist_broker, sl_dist_min_pct));
   ...
   string comment = StringFormat("WGAPv4 %s body=%.2f%% p=%.3f",
...
{
   Print("======================================================================");
   Print("  WEEKEND GAP EA v4 POSTOPEN - DIAGNOSTIC COUNTERS");
   Print("======================================================================");
   ...
   Print("  Phase 1 BE moves:            ", g_phase1_be_done);
   Print("  Phase 1 cut-loss closes:     ", g_phase1_cut_loss);
   Print("  Phase 2 close (in profit):   ", g_phase2_closed_profit);
   Print("  Phase 2 close (BE/SL hit):   ", g_phase2_closed_be);
   if(g_proba_count > 0)
   ...
   for(int i = 0; i < totalDeals; i++)
   {
      ...
      // Parse comment for proba (format: "WGAPv4 continuation body=X.XX% p=0.XXX")
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v4 POSTOPEN - BACKTEST SUMMARY");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
