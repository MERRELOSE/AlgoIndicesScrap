...
//|                         Crash 1000 - Recalibrated for 2026        |
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap Project"
#property link      ""
#property version   "5.00"
#property description "Crash 1000 v5 - Recalibrated from 7-year evolution analysis"
#property description "2026 reality: post_crash weakened, squeeze+uptrend is new king"
...
// --- Risk Management ---
input double   InpLotSize        = 1.00;     // Lot size (0.5 = min viable for Crash 1000)
...
int            magicNumber = 31000;   // Unique: Crash 1000
...
int OnInit()
{
   ...
   Print("CrashHunter v5 initialized | Symbol: ", _Symbol);
...
bool DetectEntrySignal(string &signalName, double &signalScore)
{
   ...
   // === v5.2 BLOCKS: From C1000 backtest (Jan 2024 - Apr 2026) ===

   // BLOCK: squeeze_up+cross+h4 (51 trades, 43% WR, -$3)
   if(hasSqueezeUptrend && hasBullCross && hasH4Bull && !hasRSIOS && !hasConsecRev)
   {
   ...
   // BLOCK: squeeze_up+consec_rev (no h4) (6 trades, 67% WR, -$4)
   ...
   // BLOCK: combos 4+ signaux = trop tardifs
   // rsi_os+consec_rev+overext+post_crash (10 trades, 40% WR, -$5)
   // consec_rev+overext+post_crash+h4 (1 trade, 0% WR, -$18)
   if(hasPostCrash && hasOverext && hasConsecRev)
   {
      signalName = "BLOCK:combo4_crash_overext_consec";
      return false;
   }

   // BLOCK: squeeze_up+consec_rev+h4 (36 trades, 31% WR, -$6)
   if(hasSqueezeUptrend && hasConsecRev && hasH4Bull && !hasRSIOS && !hasOverext)
   {
      signalName = "BLOCK:squeeze_consec_h4";
      return false;
   }

   // BLOCK: rsi_os+post_crash WITHOUT h4 (8 trades, 50% WR, -$65 BIGGEST LOSER)
   if(hasRSIOS && hasPostCrash && !hasH4Bull && !hasOverext)
   {
      signalName = "BLOCK:rsi_crash_no_h4";
      return false;
   }
...
void PrintSummaryReport()
{
   ...
   Print("======================================================================");
   Print("  CRASHHUNTER v5 - BACKTEST SUMMARY REPORT");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
