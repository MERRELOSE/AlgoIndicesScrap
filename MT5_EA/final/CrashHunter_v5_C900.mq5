...
bool DetectEntrySignal(string &signalName, double &signalScore)
{
   ...
   // === v5.2 BLOCKS: From C900 v5 backtest (Sep 2024 - Apr 2026) ===

   // BLOCK: squeeze_up+consec_rev+h4 (16 trades, 50% WR, -$147 BIGGEST NEW LOSER)
   if(hasSqueezeUptrend && hasConsecRev && hasH4Bull && !hasRSIOS && !hasOverext)
   {
      signalName = "BLOCK_C900v2:squeeze_consec_h4";
      return false;
   }

   // BLOCK: rsi_momentum+consec_rev+h4 (1 trade, 0% WR, -$55)
   if(hasRSIMomentum && hasConsecRev && hasH4Bull && !hasSqueezeUptrend)
   {
      signalName = "BLOCK_C900v2:momentum_consec_h4";
      return false;
   }

   // BLOCK: rsi_os+post_crash+h4 WITHOUT overext (2 trades, 0% WR, -$49)
   if(hasRSIOS && hasPostCrash && hasH4Bull && !hasOverext && !hasConsecRev)
   {
      signalName = "BLOCK_C900v2:rsi_crash_h4_only";
      return false;
   }

   // BLOCK: overext+post_crash WITHOUT h4 (1 trade, 0% WR, -$28)
   if(hasOverext && hasPostCrash && !hasH4Bull && !hasRSIOS)
   {
      signalName = "BLOCK_C900v2:overext_crash_no_h4";
      return false;
   }

   // BLOCK: combo 4+ rsi_os+consec_rev+overext+post_crash (9 trades, 56% WR, -$14)
   if(hasRSIOS && hasConsecRev && hasOverext && hasPostCrash)
   {
      signalName = "BLOCK_C900v2:combo4_all";
      return false;
   }
