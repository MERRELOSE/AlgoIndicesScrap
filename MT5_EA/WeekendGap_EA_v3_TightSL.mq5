//+------------------------------------------------------------------+
//|                              WeekendGap_EA_v3_TightSL.mq5         |
//|     Weekend-gap trader: ONNX gating + ULTRA-TIGHT cut-and-run SL  |
//|                       AlgoIndicesScrap Project                    |
//+------------------------------------------------------------------+
//  v3 = v2 with SL TIGHTENED from 0.5% to 0.10%.
//  Philosophy change:
//     v2 = "ride out small adverse gaps, hope for mean reversion"
//     v3 = "any adverse Sunday open >= 0.10% -> exit immediately at open"
//
//  Rationale (Kennedy's intuition): the gap is one-directional; if it
//  opens against us, exit at the open price with the smallest tolerable
//  loss instead of holding through more potential downside.
//
//  Empirical comparison vs v2 will tell us which philosophy wins on
//  the actual silver/gold weekend gap distribution post-2022.
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap"
#property version   "3.00"
#property description "Weekend gap EA v3 - tight SL (cut-and-run on adverse Sunday open)."
#property strict
...
input int    InpMagic           = 790030;   // v3 magic (different from v1/v2)
...
int OnInit()
{
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3 TIGHT-SL - INIT");
   Print("======================================================================");
...
void TryOpenWeekendTrade(datetime now_utc)
{
   ...
   // ORIGINAL v3: SL = 0.10% of entry price (no broker minimum check).
   // On silver $80+ (2024-26) this works fine. On silver $20-23 (2022-23) the
   // broker rejects the order ("invalid stops") because the SL is too tight.
   // -> Use this version when running on recent data (2024+) or high-priced
   //    metals. For multi-year backtests including low-priced silver, use v3.1.
   ...
   string comment = StringFormat("WGAPv3 %s body=%.2f%% p=%.3f",
...
{
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3 TIGHT-SL - DIAGNOSTIC COUNTERS");
   Print("======================================================================");
   ...
   for(int i = 0; i < totalDeals; i++)
   {
      ...
      // Parse comment for proba (format: "WGAPv3 continuation body=X.XX% p=0.XXX")
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3 TIGHT-SL - BACKTEST SUMMARY");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
