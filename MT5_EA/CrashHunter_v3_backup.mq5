...
//|                         Crash 1000 Index - Proven Signals Only    |
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap Project"
#property link      ""
#property version   "3.00"
#property description "Crash 1000 - Behavior-based EA using statistically validated patterns"
#property description "Signals: Post-crash recovery, RSI oversold, EMA cross, H4 trend filter"
...
// --- Risk Management ---
input double   InpLotSize        = 0.01;     // Lot size (start small)
input double   InpSLMultiplier   = 1.5;      // Stop Loss (x ATR)
input double   InpTPMultiplier   = 3.0;      // Take Profit (x ATR)
input double   InpTrailStart     = 1.0;      // Trailing start (x ATR profit)
input double   InpTrailDistance  = 2.0;      // Trailing distance (x ATR)
...
int OnInit()
{
   ...
   Print("CrashHunter v3 initialized | Symbol: ", _Symbol);
...
bool DetectEntrySignal(string &signalName, double &signalScore)
{
   ...
   // Signal 1: Post-crash recovery (strongest: 68% WR, +18% edge)
   ...
   // Signal 2: RSI oversold (20-35 zone only, deep oversold excluded)
   if(currentRSI >= InpRSIDeepOS && currentRSI < InpRSIOversold)
   {
      signalName += "rsi_os+";
      signalScore += 0.6;
      signalCount++;
   ...
   // Signal 3: Consecutive down reversal
   int consecDown = CountConsecDown();
   if(consecDown >= InpConsecDown && currentRSI > InpRSIDeepOS)
   {
   ...
   if(distEMA20 < -1.0 && distEMA20 > -3.0)
   {
      signalName += "overext+";
      signalScore += 0.3;
      signalCount++;
   ...
   // Signal 6: H4 bullish bonus
   if(h4Trend == 1 && signalCount > 0)
   {
      signalName += "h4_bull+";
      signalScore += 0.3;
   }
   ...
   // Entry rule: strong signal alone OR 2+ confluence signals
   if(hasStrongSignal || signalCount >= 2)
      return true;
...
void OnTick()
{
   ...
   // Wider TP for strong signals (confluence)
   if(signalScore >= 1.5)
      tp = price + (InpTPMultiplier + 1.0) * currentATR;

   // Dynamic lot size based on signal strength (optional)
...
void PrintSummaryReport()
{
   ...
   Print("======================================================================");
   Print("  CRASHHUNTER v3 - BACKTEST SUMMARY REPORT");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
