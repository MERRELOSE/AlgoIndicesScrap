//+------------------------------------------------------------------+
//|                  WeekendGap_EA_v3_1_TightSL_BrokerFix.mq5         |
//|     v3 + bid-based SL distance check (handles low-price periods)  |
//|                       AlgoIndicesScrap Project                    |
//+------------------------------------------------------------------+
//  v3.1 = v3 with smarter SL minimum check.
//  v3 (original) used SL = 0.10% of entry price. Worked on silver $80+
//  (2024-26) but the broker rejected orders on low-priced silver $20-23
//  (2022-23) because the SL was too close to the BID (spread + min_stop_level
//  exceeds 0.10%).
//
//  v3.1 fix: ensure SL is always at least (bid - (broker_min + 50pts)).
//  In recent data (silver $80+) this doesn't change anything - 0.10% is
//  already wide enough. In old data (silver $20-23), SL widens just enough
//  to satisfy the broker.
//
//  WORK IN PROGRESS - Kennedy's iterative file. v3 (the original tight)
//  is preserved for reference and recent-data backtests.
//+------------------------------------------------------------------+
#property copyright "AlgoIndicesScrap"
#property version   "3.10"
#property description "Weekend gap EA v3.1 - tight SL with bid-based broker minimum."
#property strict
...
input int    InpMagic           = 790031;   // v3.1 magic (different from v3)
...
//| HARDCODED                                                         |
//+------------------------------------------------------------------+
#define FIXED_SL_PCT          0.10     // v3: TIGHT cut-and-run SL (was 0.5 in v2)
...
int OnInit()
{
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3.1 BROKER-FIX - INIT");
   Print("======================================================================");
...
void TryOpenWeekendTrade(datetime now_utc)
{
   ...
   // ---- v3.1 SL calc ----
   // Try the tight 0.10% SL first.
   double sl_dist = entry_price * FIXED_SL_PCT / 100.0;
   double sl_price = (trade_dir > 0)
                     ? NormalizeDouble(entry_price - sl_dist, g_digits)
                     : NormalizeDouble(entry_price + sl_dist, g_digits);

   // Then verify it's far enough from the BID (broker's actual constraint).
   // For LONG : SL must be <= bid - (stops_level + safety) * point
   // For SHORT: SL must be >= ask + (stops_level + safety) * point
   long   min_stop_pts = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   double safety_dist  = (min_stop_pts + 50) * g_point;  // +50pts buffer over broker min
   if(trade_dir > 0)
   {
      double max_sl_price = bid - safety_dist;
      if(sl_price > max_sl_price)
         sl_price = NormalizeDouble(max_sl_price, g_digits);
   }
   else
   {
      double min_sl_price = ask + safety_dist;
      if(sl_price < min_sl_price)
         sl_price = NormalizeDouble(min_sl_price, g_digits);
   }
   double tp_price = 0.0;
   if(InpTPPercent > 0)
   ...
   string comment = StringFormat("WGAPv31 %s body=%.2f%% p=%.3f",
...
{
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3.1 BROKER-FIX - DIAGNOSTIC COUNTERS");
   Print("======================================================================");
   ...
   for(int i = 0; i < totalDeals; i++)
   {
      ...
      // Parse comment for proba (format: "WGAPv31 continuation body=X.XX% p=0.XXX")
   ...
   Print("======================================================================");
   Print("  WEEKEND GAP EA v3.1 BROKER-FIX - BACKTEST SUMMARY");
   Print("======================================================================");
   Print("");
   Print("--- PERFORMANCE ---");
